document.addEventListener("DOMContentLoaded", function() {
    const paises = [
        ...
        "Sierra Leona", "Singapur", "Siria", "Somalia", "Sri Lanka", "Suazilandia", "Sudán", "Sudán del Sur", 
    ...
    const selectPaisNac = document.getElementById('paisnac');
    const selectPaisRes = document.getElementById('paisres');

    // Rellenar el select de País de Nacimiento
    paises.forEach(pais => {
        const option = document.createElement('option');
        option.value = pais;
        option.textContent = pais;

        if (pais === paisSeleccionadoNac) {
    ...
    // Rellenar el select de País de Residencia
    paises.forEach(pais => {
        const option = document.createElement('option');
        option.value = pais;
        option.textContent = pais;

        if (pais === paisSeleccionadoRes) {
            option.selected = true;
        }

        selectPaisRes.appendChild(option);
    });
});
